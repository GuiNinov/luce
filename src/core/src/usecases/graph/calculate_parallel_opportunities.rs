...
impl<R: GraphRepository> CalculateParallelOpportunitiesUseCase<R> {
    ...
    pub async fn execute(
        &self,
        input: CalculateParallelOpportunitiesInput<'_>,
    ) -> Result<ParallelOpportunities, LuceError> {
        let graph = self.repository.load_graph(input.graph_id).await?;
        let ready_tasks: Vec<TaskId> = graph
            .get_ready_tasks()
            .into_iter()
            .map(|task| task.id)
            .collect();
        let available_tasks: Vec<TaskId> = graph
            .get_available_tasks()
            .into_iter()
            .map(|task| task.id)
            .collect();
    ...
    // Helper methods
    fn analyze_task_conflicts(
        &self,
        graph: &TaskGraph,
        tasks: &[TaskId],
    ) -> Result<Vec<Vec<TaskId>>, LuceError> {
