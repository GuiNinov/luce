...
impl<R: GraphRepository> OptimizeForParallelExecutionUseCase<R> {
    ...
    pub async fn execute(&self, input: OptimizeForParallelExecutionInput<'_>) -> Result<ParallelExecutionPlan, LuceError> {
        let graph = self.repository.load_graph(input.graph_id).await?;
        let _ready_tasks: Vec<TaskId> = graph.get_ready_tasks().into_iter().map(|task| task.id).collect();
