...
use chrono::Utc;
use luce_shared::{Task, TaskId, TaskStatus, LuceError};
...
#[async_trait]
impl<R: TaskRepository + Send + Sync> UseCase<UpdateTaskStatusInput, Task> for UpdateTaskStatusUseCase<R> {
...
mod tests {
    ...
    async fn create_test_repos() -> (CreateTaskUseCase<SqliteTaskRepository>, UpdateTaskStatusUseCase<SqliteTaskRepository>) {
        ...
        (CreateTaskUseCase::new(repo1), UpdateTaskStatusUseCase::new(repo2))
    }
    ...
    async fn test_update_task_status_valid_transition() {
        ...
        let task = create_usecase.execute(CreateTaskInput::new("Test task".to_string())).await.unwrap();
