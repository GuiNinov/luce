...
impl<R: TaskRepository> ListTasksUseCase<R> {
    ...
    fn apply_filter(&self, tasks: Vec<Task>, filter: &TaskFilter) -> Vec<Task> {
        match filter {
            ...
            TaskFilter::ByStatus(status) => tasks
                .into_iter()
                .filter(|task| task.status == *status)
                .collect(),
            TaskFilter::BySession(session_id) => tasks
                .into_iter()
                .filter(|task| task.assigned_session.as_deref() == Some(session_id))
                .collect(),
            TaskFilter::Unassigned => tasks
                .into_iter()
...
mod tests {
    ...
    use crate::usecases::task::update_task_status::{
        UpdateTaskStatusInput, UpdateTaskStatusUseCase,
    };
    ...
    async fn test_list_all_tasks() {
        ...
        create_usecase
            .execute(CreateTaskInput::new("Task 1".to_string()))
            .await
            .unwrap();
        create_usecase
            .execute(CreateTaskInput::new("Task 2".to_string()))
            .await
            .unwrap();
    ...
    async fn test_list_tasks_by_status() {
        ...
        let task1 = create_usecase
            .execute(CreateTaskInput::new("Task 1".to_string()))
            .await
            .unwrap();
        let task2 = create_usecase
            .execute(CreateTaskInput::new("Task 2".to_string()))
            .await
            .unwrap();

        update_usecase
            .execute(UpdateTaskStatusInput::new(task2.id, TaskStatus::Ready))
            .await
            .unwrap();
    ...
    async fn test_list_tasks_by_session() {
        ...
        assign_usecase
            .execute(AssignSessionInput::assign(
                task1.id,
