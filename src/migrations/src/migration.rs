...
impl Migration {
    ...
    pub fn get_timestamp_as_datetime(&self) -> Result<DateTime<Utc>, MigrationError> {
        ...
        let year: i32 = self.timestamp[0..4].parse()
            .map_err(|_| MigrationError::InvalidFilename { filename: self.name.clone() })?;
        let month: u32 = self.timestamp[4..6].parse()
            .map_err(|_| MigrationError::InvalidFilename { filename: self.name.clone() })?;
        let day: u32 = self.timestamp[6..8].parse()
            .map_err(|_| MigrationError::InvalidFilename { filename: self.name.clone() })?;
        let hour: u32 = self.timestamp[8..10].parse()
            .map_err(|_| MigrationError::InvalidFilename { filename: self.name.clone() })?;
        let minute: u32 = self.timestamp[10..12].parse()
            .map_err(|_| MigrationError::InvalidFilename { filename: self.name.clone() })?;
        let second: u32 = self.timestamp[12..14].parse()
            .map_err(|_| MigrationError::InvalidFilename { filename: self.name.clone() })?;

        Ok(Utc.with_ymd_and_hms(year, month, day, hour, minute, second)
            .single()
            .ok_or_else(|| MigrationError::InvalidFilename { filename: self.name.clone() })?)
    }
}
...
mod tests {
    ...
    fn test_parse_valid_filename() {
        let (timestamp, description) = Migration::parse_filename("20250320174208_first_migration.sql").unwrap();
    ...
    fn test_parse_filename_with_underscores() {
        let (timestamp, description) = Migration::parse_filename("20250320174209_create_user_table.sql").unwrap();
