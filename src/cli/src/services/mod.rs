...
mod tests {
    ...
    async fn test_service_list_tasks() {
        ...
        // Create a few tasks
        service.create_task("Task 1".to_string(), None, None).await.unwrap();
        service.create_task("Task 2".to_string(), None, None).await.unwrap();
    ...
    async fn test_service_update_task_status() {
        ...
        let task = service.create_task("Test task".to_string(), None, None).await.unwrap();
        
        service.update_task_status(&task.id.to_string(), luce_shared::TaskStatus::Completed)
            .await.unwrap();
